...
async function processLineByLine() {
	...
	for await (const line of rl) {
		...
		for (let i = 1; i < parts.length; i++) {
			...
			rowMax = Math.max(rowMax, p0[1], p1[1]);
	...
	const fall = (row, column, count) => {
		while(row < rowMax + 2 && (map[column] === undefined || map[column][row] === undefined)) {
			row++;
		}
		...
		if (row === rowMax + 2) {
			...
			if (map[column-1] === undefined || map[column-1][row] === undefined) {
				return fall(row, column - 1, count);
			} else if (map[column+1] === undefined || map[column+1][row] === undefined) {
				return fall(row, column + 1, count);
			} else {
				map[column] = map[column] || {};
				map[column][row-1] = "*";
